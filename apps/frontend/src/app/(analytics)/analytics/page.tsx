...
export default function AnalyticsPage() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const res = await fetch(`${apiURL}/llm/analytics`, {
        method: "GET",
        headers: getAuthHeaders(),
        cache: "no-store", // Her zaman canlı veriyi çek!
      });
      if (res.ok) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-neutral-900/60 border border-neutral-800 p-5 rounded-2xl space-y-2">
          ...
          <p className="text-[11px] text-neutral-500">Kabul & Reddedilenler</p>
        </div>
        ...
        <div className="bg-neutral-900/60 border border-neutral-800 p-5 rounded-2xl space-y-2">
          ...
          <p className="text-[11px] text-neutral-500">Edge AI Çıkarım Süresi</p>
        </div>

        {/* Card 3: Decision Score */}
        <div className="bg-neutral-900/60 border border-neutral-800 p-5 rounded-2xl space-y-2">
          <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">
            Ortalama Kabul Skoru
          </p>
          <p className="text-3xl font-bold text-emerald-400 font-mono">
            {summary.avg_score.toFixed(2)} <span className="text-sm font-normal text-neutral-400">/ 5.0</span>
          </p>
          <p className="text-[11px] text-neutral-500">Yönetmen Değerlendirmesi</p>
        </div>

        {/* Card 4: Puanlanan Oranı */}
        <div className="bg-neutral-900/60 border border-neutral-800 p-5 rounded-2xl space-y-2">
          <p className="text-xs font-semibold text-neutral-400 uppercase tracking-wider">
            Skorlanmış Replikler
          </p>
          ...
          <p className="text-[11px] text-neutral-500">Sahneye Eklenenler</p>
        </div>
      ...
      <div className="bg-neutral-900/40 border border-neutral-800 p-6 rounded-2xl space-y-4">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold text-white">🏆 En İyi Sahne Replikleri (Leaderboard)</h2>
          <button
            ...
            data.top_logs.map((log) => (
              ...
              >
                <div className="space-y-1 flex-1">
                  ...
                  <p className="text-sm text-neutral-200 line-clamp-2">{log.response}</p>
                </div>
                ...
                <div className="bg-neutral-900 border border-neutral-800 px-3 py-2 rounded-xl text-center self-start sm:self-auto min-w-[100px]">
                  <span className="text-[10px] text-neutral-500 block uppercase">Yönetmen Puanı</span>
